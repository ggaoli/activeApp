                            ...
                            for (j = i + 1; j < _blocks.length; j++) {
                                ...
                                if (_blocks[j].indexOf("{{#" + helperName) >= 0) {
                                    ...
                                    if (_blocks[j].indexOf("{{/" + helperName) >= 0) {
                                        ...
                                        if (_blocks[j].indexOf("else") >= 0 && depth === 0) {
                                            foundElse = true
                                        } else {
                                            if (!foundElse) {
                                                helperContent += _blocks[j]
...
(function(window, document, exportName, undefined) {
    ...
    function getTouchActionProps() {
        ...
        var touchMap = {};
        var cssSupports = window.CSS && window.CSS.supports; ["auto", "manipulation", "pan-y", "pan-x", "pan-x pan-y", "none"].forEach(function(val) {
    ...
    function($) {
        ...
        $.login = function(text, title, onOK, onCancel, username, password) {
            ...
            var modal = $.modal({
                    text: '<p class="weui-prompt-text">' + (config.text || "") + "</p>" + '<input type="text" class="weui_input weui-prompt-input" id="weui-prompt-username" value="' + (config.username || "") + '" placeholder="输入用户名" />' + '<input type="password" class="weui_input weui-prompt-input" id="weui-prompt-password" value="' + (config.password || "") + '" placeholder="输入密码" />',
                    title: config.title,
                    autoClose: false,
                    buttons: [{
                        text: defaults.buttonCancel,
    ...
    function($) {
        ...
        $.showTab = showTab;
        $(document).on("click", ".weui_tabbar_item, .weui_navbar_item",
            function(e) {
    ...
    function($) {
        $(document).on("click", ".weui_search_bar label",
            function(e) {
            ...
            function(e) {
                var $input = $(e.target);
                if (!$input.val()) {
                    $input.parents(".weui_search_bar").removeClass("weui_search_focusing")
                }
    ...
    function($) {
        var Picker = function(params) {
            ...
            p.layout = function() {
                ...
                pickerHTML = '<div class="' + (pickerClass) + '">' + (p.params.toolbar ? p.params.toolbarTemplate.replace(/{{closeText}}/g, p.params.toolbarCloseText).replace(/{{title}}/g, p.params.title) : "") + '<div class="picker-modal-inner picker-items">' + colsHTML + '<div class="picker-center-highlight"></div>' + "</div>" + "</div>";
        ...
        $(document).on($.touchEvents.move, ".picker-modal-inner",
            function(e) {
                e.preventDefault()
            });
    ...
    function($) {
        ...
        $(document).on("click", ".close-popup, .weui-popup-overlay",
            function() {
                $.closePopup()
            }).on("click", ".open-popup",
            function() {
            ...
            function(e) {
                if ($(e.target).hasClass("weui-popup-container")) {
                    $.closePopup()
                }
            });
        $.fn.popup = function() {
